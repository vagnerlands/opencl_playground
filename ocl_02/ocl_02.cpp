...
double timeAddVectorsCPU() {
    ...
    std::shared_ptr<int> B(new int[n], std::default_delete<int[]>());
    std::shared_ptr<int> C(new int[n], std::default_delete<int[]>());
    ...
    for (int i=0; i<k; i++) {
        for (int j = 0; j < n; j++)
        {
            C.get()[j] = A.get()[j] + B.get()[j];
            sum += C.get()[j];
        }
    }
    duration = (std::clock() - start) / (double) CLOCKS_PER_SEC;
    //std::cout << " val " << std::to_string(sum) << std::endl;
    return duration;
}


void warmup(cl::Context &context, cl::CommandQueue &queue, 
            cl::Kernel &add, int* A, int* B) {
    std::shared_ptr<int> C(new int[n], std::default_delete<int[]>());
    // allocate space
    ...
    queue.enqueueReadBuffer(buffer_C, CL_TRUE, 0, sizeof(int)*n, C.get()); 
    queue.finish(); 
}
...
int main(int argc, char* argv[]) {
    ...
    // use device[1] because that's a GPU; device[0] is the on-board GPU
    ...
    // run the CPU code
    float CPUtime = timeAddVectorsCPU();
    ...
    // attempt at warm-up...
    warmup(context, queue, add, A.get(), B.get());
    queue.finish();
    ...
    // push write commands to queue
    queue.enqueueWriteBuffer(buffer_A, CL_TRUE, 0, sizeof(int)*n, A.get());
    queue.enqueueWriteBuffer(buffer_B, CL_TRUE, 0, sizeof(int)*n, B.get());
    ...
    queue.enqueueWriteBuffer(buffer_A2, CL_TRUE, 0, sizeof(int)*n, A.get());
    queue.enqueueWriteBuffer(buffer_B2, CL_TRUE, 0, sizeof(int)*n, B.get());
    ...
    queue.enqueueReadBuffer(buffer_C2, CL_TRUE, 0, sizeof(int)*n, C.get());
    queue.finish();
